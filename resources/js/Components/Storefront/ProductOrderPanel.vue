...
<template>
    <section v-if="product.unit_price" class="mt-7 rounded-lg border border-tabarak-line bg-tabarak-mist p-4 sm:p-5" aria-labelledby="order-product">
        <div class="flex flex-wrap items-end justify-between gap-4">
            ...
            <form v-if="product.is_available" class="flex flex-wrap items-end gap-2" @submit.prevent="add">
                <label>
                    ...
                    <input v-model.number="form.quantity" class="field-input w-24" type="number" min="1" :max="maximumQuantity" required />
                </label>
        ...
        <p v-if="product.tracks_stock && product.is_available" class="mt-3 text-xs font-semibold text-slate-500">{{ product.stock_quantity }} available</p>
