...
const imageOptions = computed<SelectOption[]>(() => [{ value: '', label: 'No logo' }, ...props.media.map((item) => ({ value: item.id, label: item.original_name || `Image #${item.id}` }))]);
...
<template>
    <form class="space-y-4" @submit.prevent="submit">
        ...
        <label class="block"><span class="field-label">Logo image</span><SelectMenu v-model="form.logo_image_id" :options="imageOptions" aria-label="Brand logo" /></label>
