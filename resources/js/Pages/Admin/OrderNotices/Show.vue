...
import { ArrowLeft, RotateCcw, ShieldCheck } from '@lucide/vue';
    ...
    <AdminLayout>
        ...
        <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px] xl:items-start">
            ...
            <aside class="space-y-4 xl:sticky xl:top-24">
                <section class="surface p-5"><p class="eyebrow">Customer</p><h2 class="mt-1 text-xl font-bold text-tabarak-ink">{{ notice.customer_name }}</h2><p class="mt-2 text-sm font-bold text-tabarak-blue">{{ notice.customer_phone }}</p><dl class="mt-5 space-y-3 border-t border-tabarak-line pt-4 text-sm"><div class="flex justify-between gap-3"><dt class="text-slate-500">Original status</dt><dd class="font-semibold capitalize">{{ notice.order_status }}</dd></div><div class="flex justify-between gap-3"><dt class="text-slate-500">Submitted</dt><dd class="text-right font-semibold">{{ formatDateTime(notice.submitted_at) }}</dd></div><div class="flex justify-between gap-3"><dt class="text-slate-500">Deleted by</dt><dd class="text-right font-semibold">{{ notice.deleted_by || 'Former administrator' }}</dd></div></dl></section>
